// Press Shift twice to open the Search Everywhere dialog and type `show whitespaces`,
// then press Enter. You can now see whitespace characters in your code.
public class Main {
    public static void main(String[] args) {
        //task1();//накопления
        //task2();//счет
        //task3();//численность населения
        //task4();//накопления с процентами
        //task5();
        //task6();
        //task7();
        task8();
    }

    private static void task1() {
        System.out.println("Задача 1");
        int accumulation = 15000;
        int goal = 2459000;
        int total = 0;
        int i = 0;
        while (total < goal) {
            total += accumulation;
            i++;
            String msg = String.format("Месяц %s, сумма накоплений равна %s рублей", i, total);
            System.out.println(msg);
        }
        System.out.println("Потребовалось месяцев: " + i);
    }

    private static void task2() {
        System.out.println("Задача 2");
        int i = 1;
        while (i <= 10) {
            System.out.print(i + " ");
            i++;
        }
        System.out.println();
        for (int j = 10; j > 0; j--) {
            System.out.print(j + " ");
        }
    }

    private static void task3() {
        System.out.println("Задача 3");
        int population = 12000000;
        int birthIncr = 17;
        int deathDecr = 8;
        int temp = 10;
        int year = 1;
        while (year <= temp) {
            population += population * (birthIncr - deathDecr) / 1000;
            System.out.println(String.format("Год %s, численность населения составляет %s", year, population));
            year++;
        }
    }

    private static void task4() {
        System.out.println("Задача 4");
        double sum = 15000;
        double goal = 12000000;
        int month = 1;
        int percent = 7;
        while (sum < goal) {
            sum += sum * percent / 100;
            System.out.println(String.format("Месяц %s, сумма накопления составляет %s", month, sum));
            month++;
        }
    }

    private static void task5() {
        System.out.println("Задача 5");
        double sum = 15000;
        double goal = 12000000;
        int month = 1;
        int percent = 7;
        while (sum < goal) {
            sum += sum * percent / 100;
            if (month % 6 == 0) {
                System.out.println(String.format("Месяц %s, сумма накопления составляет %s", month, sum));
            }
            month++;
        }
    }

    private static void task6(){
        System.out.println("Задача 6");
        double sum = 15000;
        double yearCount = 9;
        int month = 1;
        int percent = 7;
        while (month < yearCount * 12) {
            sum += sum * percent / 100;
            if (month % 6 == 0) {
                System.out.println(String.format("Месяц %s, сумма накопления составляет %s", month, sum));
            }
            month++;
        }
    }

    private static void task7(){
        System.out.println("Задача 7");
        int friday = 4;
        int dayInMonth = 31;
        int dayInWeek = 7;
        for (int i = friday; i < dayInMonth; i += dayInWeek){
            System.out.println(String.format("Сегодня пятница, %s-е число. Необходимо составить отчет", i));
        }

    }

    private static void task8(){
        System.out.println("Задача 8");
        int cometFrequency = 79;
        int prevTemp = 200;
        int nextTemp = 100;
        int currentYear = 2023;
        for (int i = currentYear - prevTemp; i < currentYear + nextTemp; i++){
            if (i % cometFrequency == 0){
                System.out.println(i);
            }
        }
    }
}